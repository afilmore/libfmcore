...
FmMimeType *fm_mime_type_new (const char *type_name)
{
    ...
    GIcon *gicon = g_content_type_get_icon (mime_type->type);
...
FmMimeType *fm_mime_type_get_for_native_file (const char *file_path, const char *base_name, struct stat *pstat)
{
    ...
    if (S_ISREG (pstat->st_mode))
    {
        ...
        if (uncertain)
        {
            ...
            // Empty file means a text file with 0 characters in it.
            if (pstat->st_size == 0)
    ...
    /***
     * Some files under /proc/self has st_mode = 0, which causes problems,
