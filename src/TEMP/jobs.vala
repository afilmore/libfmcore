...
//~         CopyJob job = new CopyJob(CopyJobMode.COPY, path_list, dest_paths, ui);
//~         job.run_async();
//~         return job;
//~     }
...
//~         
//~         PathList dest_paths = new PathList();
//~         
//~         foreach(unowned Path src_path in path_list.peek_head_link()) {
//~             
//~             Path dest_path = new Path.child(dest_dir, src_path.get_basename());
...
//~ 
    ...
    /*public FileJob move_files2(PathList path_list, PathList dest_paths, FileJobUI? ui = null) {
        CopyJob job = new CopyJob(CopyJobMode.MOVE, path_list, dest_paths, ui);
        job.run_async();
        return job;
    }
