...
static gboolean fm_folder_model_iter_nth_child (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent,
                                                gint n);
static gboolean fm_folder_model_iter_parent (GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *child);
static gboolean fm_folder_model_get_sort_column_id (GtkTreeSortable *sortable, gint *sort_column_id,
                                                    GtkSortType *order);
static void fm_folder_model_set_sort_column_id (GtkTreeSortable *sortable, gint sort_column_id, GtkSortType order);

#if !ENABLE_GTK3
static void fm_folder_model_set_sort_func (GtkTreeSortable *sortable, gint sort_column_id,
                                           GtkTreeIterCompareFunc sort_func, gpointer user_data,
                                           GtkDestroyNotify destroy);
static void fm_folder_model_set_default_sort_func (GtkTreeSortable *sortable, GtkTreeIterCompareFunc sort_func,
...
static inline FmFolderItem *fm_folder_item_new (FmFileInfo *file_info)
{
    FmFolderItem *folder_item = g_slice_new0 (FmFolderItem);
    
    folder_item->file_info = fm_file_info_ref (file_info);
...
static inline void fm_folder_item_free (FmFolderItem *folder_item)
{
    ...
    fm_file_info_unref (folder_item->file_info);
...
static void _fm_folder_model_add_file (FmFolderModel *model, FmFileInfo *file_info)
{
    if (!model->show_hidden && fm_file_info_is_hidden (file_info))
        g_sequence_append (model->hidden, fm_folder_item_new (file_info));
    else
        fm_folder_model_file_created (model, file_info);
}
...
GtkTreePath *fm_folder_model_get_path (GtkTreeModel *tree_model, GtkTreeIter *iter)
{
    GtkTreePath *path;
    GSequenceIter *items_it;
                               ...
                               GValue *value)
{
    ...
    FmFileInfo *file_info = folder_item->file_info;

    switch (column)
    {
    
        case COL_FILE_GICON:
            g_value_set_object (value, fm_file_info_get_fm_icon (file_info)->gicon);
        break;
        ...
        case COL_FILE_ICON:
        {
            ...
            {
                if (!fm_file_info_get_fm_icon (file_info))
                    return;
                
                folder_item->icon = fm_icon_get_pixbuf (fm_file_info_get_fm_icon (file_info), model->icon_size);
            }
            ...
            if (fm_config->show_thumbnail && (fm_path_is_local (path) || !fm_config->thumbnail_local))
            {
                ...
                {
                    if (fm_file_info_can_thumbnail (folder_item->file_info))
                    {
                        if (folder_item->file_info->size > 0 && folder_item->file_info->size <= (fm_config->thumbnail_max << 10))
                        {
                            FmThumbnailRequest *req = fm_thumbnail_request (folder_item->file_info, model->icon_size, on_thumbnail_loaded, model);
        ...
        case COL_FILE_NAME:
            g_value_set_string (value, file_info->disp_name);
        break;
        
        case COL_FILE_SIZE:
            g_value_set_string (value, fm_file_info_get_disp_size (file_info));
        break;
        
        case COL_FILE_DESC:
            g_value_set_string (value, fm_file_info_get_desc (file_info));
        break;
        
        case COL_FILE_PERM:
            // g_value_set_string (value, fm_file_info_get_disp_perm (file_info));
        break;
        
        case COL_FILE_OWNER:
            // g_value_set_string (value, fm_file_info_get_disp_owner (file_info));
        break;
        
        case COL_FILE_MTIME:
            g_value_set_string (value, fm_file_info_get_disp_mtime (file_info));
        break;
        
        case COL_FILE_INFO:
            g_value_set_pointer (value, file_info);
        break;
    }
}
...
static gint fm_folder_model_compare (FmFolderItem *item1,
                                    ...
                                    FmFolderModel *model)
{
    FmFileInfo *file1 = item1->file_info;
    FmFileInfo *file2 = item2->file_info;
    const char *key1;
    const char *key2;
    int ret = 0;
...
void fm_folder_model_sort (FmFolderModel *model)
{
    ...
    if (!old_order)
        DEBUG ("fm_folder_model_sort: hash table = NULL !!!\n");
    else
...
void fm_folder_model_file_created (FmFolderModel *model, FmFileInfo *file_info)
{
    FmFolderItem *new_item = fm_folder_item_new (file_info);
                                  ...
                                  FmFolderModel *model)
{
    ...
    FmFileInfo *file_info = new_item->file_info;
...
void fm_folder_model_file_deleted (FmFolderModel *model, FmFileInfo *file_info)
{
    ...
    if (G_UNLIKELY (!file_info))
    {
        ...
        while (!g_sequence_iter_is_end (items_it))
        {
            ...
            file_info  = (VFSFileInfo*)g_sequence_get (items_it);
            items_it = g_sequence_iter_next (it);
            vfs_file_info_unref (file_info);
        }
        for (l = model->items; l; l = model->items)
        {
            gtk_tree_model_row_deleted (GTK_TREE_MODEL (model), path);
            file_info = (VFSFileInfo*)l->data;
    ...
    if (!model->show_hidden && fm_file_info_is_hidden (file_info)) /* if this is a hidden file */
    {
        update_view = FALSE;
    ...
    while (!g_sequence_iter_is_end (seq_it))
    {
        ...
        if (folder_item->file_info == file_info)
            break;
...
void fm_folder_model_file_changed (FmFolderModel *model, FmFileInfo *file_info)
{
    FmFolderItem *folder_item;
    GSequenceIter *items_it;
    GtkTreeIter it;
...
void fm_folder_model_set_show_hidden (FmFolderModel *model, gboolean show_hidden)
{
    ...
    else /* move invisible items to hidden list */
    {
        ...
        while (!g_sequence_iter_is_end (items_it))
        {
            ...
            folder_item = (FmFolderItem*)g_sequence_get (items_it);
            if (fm_file_info_is_hidden (folder_item->file_info))
            {
                                                  ...
                                                  gchar *common_suffix)
{
    ...
    {
        FmFolderItem *folder_item = (FmFolderItem*)g_sequence_get (item_it);
        gboolean predicate_ok = (file_info_predicate == NULL) || file_info_predicate (folder_item->file_info);
        gint i = 0;
        if (predicate_ok && g_str_has_prefix (folder_item->file_info->disp_name, prefix))
        {
            /* first match -> init */
            if (!common_suffix_initialized)
            {
                strcpy (common_suffix,  folder_item->file_info->disp_name + prefix_len);
                common_suffix_initialized = TRUE;
            }
            else
            {
                while (common_suffix[i] == folder_item->file_info->disp_name[prefix_len + i])
                    i++;
                common_suffix[i] = 0;
            }
...
static gboolean fm_folder_model_find_iter_by_fileinfo (FmFolderModel *model, GtkTreeIter *it, FmFileInfo *file_info)
{
    GSequenceIter *item_it = g_sequence_get_begin_iter (model->items);
    for ( ; !g_sequence_iter_is_end (item_it); item_it = g_sequence_iter_next (item_it))
    {
...
static GList *find_in_pending_thumbnail_requests (FmFolderModel *model, FmFileInfo *file_info)
{
    ...
    for (l=reqs;l;l=l->next)
    {
        ...
        if (0 == g_strcmp0 (file_info->path->name, fi2->path->name))
            return l;
    }
    return NULL;
}
...
void on_thumbnail_local_changed (FmConfig *cfg, gpointer user_data)
{
    ...
    if (cfg->thumbnail_local)
    {
        ...
        for (l = model->thumbnail_requests; l;)
        {
            ...
            file_info = fm_thumbnail_request_get_file_info (req);
            if (!fm_path_is_local (file_info->path))
            {
    ...
    {
        FmFolderItem *folder_item = (FmFolderItem*)g_sequence_get (seq_it);
        file_info = folder_item->file_info;
        if (cfg->thumbnail_local)
        {
        ...
        else
        {
            ...
            if (!fm_path_is_local (file_info->path))
            {
                req = fm_thumbnail_request (file_info, model->icon_size, on_thumbnail_loaded, model);
...
void on_thumbnail_max_changed (FmConfig *cfg, gpointer user_data)
{
    ...
    if (cfg->thumbnail_max)
    {
        ...
        for (l = model->thumbnail_requests; l; )
        {
            ...
            req = (FmThumbnailRequest*)l->data;
            file_info = fm_thumbnail_request_get_file_info (req);
            if (file_info->size > (cfg->thumbnail_max << 10))
            {
                fm_thumbnail_request_cancel (req);
                model->thumbnail_requests = g_list_delete_link (model->thumbnail_requests, l);
            }
    ...
    while (!g_sequence_iter_is_end (seq_it))
    {
        ...
        if (cfg->thumbnail_max)
        {
            ...
            {
                if (file_info->size < thumbnail_max_bytes && file_info->size > model->thumbnail_max)
                {
                    if (!folder_item->thumbnail_failed && fm_file_info_can_thumbnail (file_info))
                    {
                        req = fm_thumbnail_request (file_info, model->icon_size, on_thumbnail_loaded, model);
                        new_reqs = g_list_append (new_reqs, req);
                    }
            ...
            {
                if (file_info->size > thumbnail_max_bytes)
                    reload_thumbnail (model, seq_it, folder_item);
            }
        }
        else /* no limit, all files can be added */
        {
            ...
            if (!folder_item->is_thumbnail && !folder_item->thumbnail_loading && !folder_item->thumbnail_failed && fm_file_info_can_thumbnail (file_info))
            {
                GList *l = find_in_pending_thumbnail_requests (model, file_info);
                if (!l)
                {
                    req = fm_thumbnail_request (file_info, model->icon_size, on_thumbnail_loaded, model);
                    new_reqs = g_list_append (new_reqs, req);
