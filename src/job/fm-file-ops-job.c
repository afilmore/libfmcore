...
{
	fm_job_call_main_thread (FM_JOB (job), (FmJobCallMainThreadFunc) emit_cur_file, (gpointer) cur_file);
}
...
void fm_file_ops_job_emit_percent (FmFileOpsJob* job)
{
    ...
    {
    	fm_job_call_main_thread (FM_JOB (job), (FmJobCallMainThreadFunc) emit_percent,  (gpointer)percent);
        job->percent = percent;
    }
}
...
{
    fm_job_call_main_thread (FM_JOB (job), (FmJobCallMainThreadFunc) emit_prepared, NULL);
}
...
FmFileOpOption fm_file_ops_job_ask_rename (FmFileOpsJob* job, GFile* src, GFileInfo* src_inf, GFile* dest, GFile** new_dest)
{
    ...
    FmFileInfoJob* fijob = (FmFileInfoJob*) fm_file_info_job_new (NULL, 0);
    ...
    data.new_name = NULL;
    fm_job_call_main_thread (FM_JOB (job), (FmJobCallMainThreadFunc) emit_ask_rename,  (gpointer)&data);
